...
**Status: shipped.** The renderer was migrated to this language 2026-06-24 (17 slices) and retuned to the black-glass palette 2026-07-02 (`styles.css` is the implementation of the token table below; main-side data-URL windows mirror it via `src/main/window-palette.ts`; the detached Chat and Captions windows are real glass — same underlay as the main window). Do not restyle screens ad hoc — fix tokens, not components.
...
Implemented as shadcn CSS variables in `apps/desktop/src/renderer/src/styles.css` (oklch; that file is the live source of truth). Main-process data-URL windows (Notes/Preview — dark-always, they frame video) mirror the solid values in `src/main/window-palette.ts`; change them together. The detached Chat and Captions windows are renderer windows: dark-always too, but black GLASS on macOS (`auxWindowChromeOptions` in `src/main/index.ts` + `GlassWallpaperUnderlay`), with the palette base only as the non-macOS / opt-out fallback.
